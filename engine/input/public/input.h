...
static_assert(static_cast<int>(Key::COUNT) == 512);
...
struct KeyboardState {
    std::array<ButtonState, static_cast<std::size_t>(Key::COUNT)> states{};
};

struct MouseState {
    std::array<ButtonState, static_cast<std::size_t>(MouseButton::COUNT)> states{};
    Vec2f position{};
    Vec2f deltaPosition{};
    Vec2f wheelDelta{};
};
...
class Input {
  private:
    ...
    void addMouseMotion(Vec2f position, Vec2f deltaPosition);
    void addWheelDelta(Vec2f delta);
    void clearTransientState();
    void cancelState();

  public:
    bool isPressed(Key key) const;
    bool isHold(Key key) const;
    bool isReleased(Key key) const;
    bool isPressed(MouseButton button) const;
    bool isHold(MouseButton button) const;
    bool isReleased(MouseButton button) const;
    Vec2f mousePosition() const noexcept;
    Vec2f mouseDeltaPosition() const noexcept;
    Vec2f mouseWheelDelta() const noexcept;
};
